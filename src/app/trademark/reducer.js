...
import {
    CHANGE_CURRENT_PAGE,
    CHANGE_INPUT_CREATE_TRADEMARK, CHANGE_SORT_BY, CREATE_NEW_TRADEMARK_SUCCESS, DELETE_TRADEMARK_SUCCESS,
    FETCH_LIST_TRADEMARKS_SUCCESS, FETCH_STATISTIC_SUCCESS, FETCH_TOTAL_WARNINGS_SUCCESS,
    FETCH_TRADEMARK_DETAIL_SUCCESS, HIDE_TRADEMARK_RESULTS, MARK_READ_TRADEMARK_SUCCESS, REFRESH_TRADEMARK_SUCCESS,
    SHOW_TRADEMARK_RESULTS,TOGGLE_UPGRADE_POPUP,OPEN_TRADEMARK_RESULTS,CHANGE_TOTAL_TRIAL
} from "./actionTypes"
...
export default createReducer(fromJS(initState), {
    ...
    [CHANGE_TOTAL_TRIAL](state) {
        return state.setIn(['statistic', 'max'], 3)
    },
    ...
    [FETCH_LIST_TRADEMARKS_SUCCESS](state, action) {
        ...
        return state
            .set('items', fromJS(docs))
            .set('meta', state.get('meta').merge(fromJS({
                    totalPages: pages,
                    limit,
                    page,
                    total
    ...
    [SHOW_TRADEMARK_RESULTS](state, action) {
        ...
        return state
            .setIn(['preview', 'item'], id)
            .setIn(['preview', 'isOpen'], true)
    },
    ...
    [MARK_READ_TRADEMARK_SUCCESS](state, action) {
        ...
        return state.set('items', items)
            .set('totalWarnings', state.get('totalWarnings') - 1)
    }
})
