...
class ButtonSubscription extends Component {
    render() {
        ...
        return (
            <div className={classNames('ButtonSubscription', planId)}>
                {
                    !isAuthenticated ? this._renderGoToLogin() :
                        <Button tag={Link} to={`/a/subscribe/${planId}`} color="primary">
                            <span><FormattedMessage id="pricing.subscribe"/></span>
                        </Button>
                }
            </div>
        );
    ...
    _renderGoToLogin() {
        ...
        return (
            <Button color="success" tag={Link} to={`/login?redirectTo=${redirectTo}`}><FormattedMessage id="pricing.subscribe"/></Button>
        );
    }
}
